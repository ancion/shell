...
backup() {
  HISTORY_DIR=${BASE_DIR}/historyVersion/ditialBack_${VERSION}
  mkdir ${HISTORY_DIR}
...
start() {
  ...
  # 启动 base 程序
  nohup java -jar -Dspring.profiles.active=prod -Dloader.path=./lib -Ddubbo.application.logger=slf4j odn-base-service-0.0.1-SNAPSHOT.jar >logs/odn-base-service-`date +%Y-%m-%d`.out 2>&1 &
  echo "启动 base 程序................."
  sleep 20

  # 启动 eqp 程序
  nohup java -server -Xmx4G -Xms1G -Dspring.profiles.active=prod -jar -Dloader.path=./lib -Ddubbo.application.logger=slf4j  odn-equipment-service-0.0.1-SNAPSHOT.jar >logs/odn-equipment-service-`date +%Y-%m-%d`.out 2>&1 &
  echo "启动 eqp 程序.................."
  sleep 30

  # 启动 order 程序
  nohup java -jar -Dspring.profiles.active=prod -Dloader.path=./lib -Ddubbo.application.logger=slf4j odn-order-service-0.0.1-SNAPSHOT.jar >logs/odn-order-service-`date +%Y-%m-%d`.out 2>&1 &
  echo "启动 order 程序................"
  sleep 20
